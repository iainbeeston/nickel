...
describe Nickel::Occurrence do
  describe "#==" do
    let(:occ) { Nickel::Occurrence.new(type: :daily, start_date: Nickel::ZDate.new("20140128")) }
    ...
    it "is false when comparing to an occurence with different dates" do
      expect(occ).to_not eq Nickel::Occurrence.new(type: :daily, start_date: Nickel::ZDate.new("20080825"))
    end

    it "is false when comparing to an occurrence with a different type" do
      expect(occ).to_not eq Nickel::Occurrence.new(type: :weekly, start_date: Nickel::ZDate.new("20140128"))
    end

    it "is false when comparing to an occurence with a different interval" do
      expect(occ).to_not eq Nickel::Occurrence.new(type: :daily, start_date: Nickel::ZDate.new("20140128"), interval: 3)
    end

    it "is true when comparing to an occurence with the same values" do
      expect(occ).to eq Nickel::Occurrence.new(type: :daily, start_date: Nickel::ZDate.new("20140128"))
    end
  end
end
